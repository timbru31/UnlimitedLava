...
public class UnlimitedLavaCheck {
    ...
    public boolean checkLavaSpreadValidity(Block block) {
        /*
         * NEW Consolidated check code. Returns true if block is fillable, false
         * if block is not fillable. Our previous methods read several blocks
         * multiple times and the test logic is duplicated in two places. This
         * processing time could be better spent elsewhere. Previous code was
         * duplicated between UnlimitedLavaBlockListener and
         * UnlimitedLavaPlayerListener. Consolidation of these tests allows
         * changes to be made in one place. checkLavaSpreadValidity is intended
         * to reduce redundancy and improve efficiency over our previous code.
         */
        ...
        Block[] blockRelative = { block.getRelative(BlockFace.NORTH),
                ...
                block.getRelative(BlockFace.NORTH, 2),
                block.getRelative(BlockFace.NORTH_NORTH_EAST),
                block.getRelative(BlockFace.NORTH_EAST, 2),
                block.getRelative(BlockFace.EAST_NORTH_EAST),
                block.getRelative(BlockFace.EAST, 2),
                block.getRelative(BlockFace.EAST_SOUTH_EAST),
                block.getRelative(BlockFace.SOUTH_EAST, 2),
                block.getRelative(BlockFace.SOUTH_SOUTH_EAST),
                block.getRelative(BlockFace.SOUTH, 2),
                block.getRelative(BlockFace.SOUTH_SOUTH_WEST),
                block.getRelative(BlockFace.SOUTH_WEST, 2),
                block.getRelative(BlockFace.WEST_SOUTH_WEST),
                block.getRelative(BlockFace.WEST, 2),
                block.getRelative(BlockFace.WEST_NORTH_WEST),
                block.getRelative(BlockFace.NORTH_WEST, 2),
        ...
        int[][] lBlock = { { 8, 9, 10, 11, 12 }, { 12, 13, 14, 15, 16 },
                { 16, 17, 18, 19, 20 }, { 20, 21, 22, 23, 8 } };
        int[][] rBlock = { { 6, 7, 23, 8, 9, 1, 2 },
                { 0, 8, 9, 10, 11, 12, 2 }, { 0, 1, 11, 12, 13, 3, 4 },
                { 2, 12, 13, 14, 15, 16, 4 }, { 2, 3, 15, 16, 17, 5, 6 },
                { 4, 16, 17, 18, 19, 20, 6 }, { 4, 5, 19, 20, 21, 7, 0 },
                { 6, 20, 21, 22, 23, 8, 0 } };
        ...
        /*
         * Evaluations Faces and Borders: Faces are full blocks of lava one
         * block N, S, E, or W of the target. Borders are any solid block (not
         * liquid or air) one block N, S, E, or W of the target. A diagonal
         * solid is not considered a border because it would not prevent flow
        ...
        for (int i = 0; i <= 3; i++) {
            ...
            /*
             * Corners and Lake Corners are three contiguous full lava blocks
             * (face, diagonal, face) and are required to further test for a
             * lake. A Lake is valid when at least 4 blocks contiguous to the
             * tested corner contain lava of any amount.
             */
            cBlocks = 0;
            for (int c = 0; c <= 2; c++) {
